...
export default function PuzzleHeader({ title, author, date: dateProp }: PuzzleHeaderProps) {
  ...
  // Parse date
  const dateStr = dateProp || titlePieces[1] || '';
  const date = dateStr ? new Date(dateStr) : new Date();
  const isValidDate = !isNaN(date.getTime());

  // Format date parts
  const dayOfWeek = isValidDate ? date.toLocaleDateString('en-US', { weekday: 'long' }) : '';
